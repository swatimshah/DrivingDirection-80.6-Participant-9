...
# Create the model  ################## BEST ONE - SO FAR - 9th participant - 2 sec samples #######################
...
model=Sequential()
model.add(Conv1D(filters=55, kernel_size=8, kernel_regularizer=L2(my_L2), bias_regularizer=L2(my_L2), activity_regularizer = L2(my_L2), kernel_constraint=min_max_norm(min_value=negative_wt, max_value=positive_wt), padding='valid', activation='relu', strides=2, input_shape=(500, 64)))
model.add(AveragePooling1D(pool_size=3)) 
model.add(Conv1D(filters=55, kernel_size=8, kernel_regularizer=L2(my_L2), bias_regularizer=L2(my_L2), activity_regularizer = L2(my_L2), kernel_constraint=min_max_norm(min_value=negative_wt, max_value=positive_wt), padding='valid', activation='relu', strides=1))
...
hist = model.fit(input, Y_train, batch_size=32, epochs=30, verbose=1, validation_data=(testinput, Y_test), steps_per_epoch=None, callbacks=[es, mc])
